...
KKS_AccessoryBoneBinder is a BepInEx plugin for Koikatsu Sunshine that attaches selected accessory bone chains directly to matching character body bones at runtime. It uses `BoneImplantProcess` markers authored with ModBoneImplantor and does not require AccessoryClothes.
...
- Koikatsu Sunshine
- BepInEx
- KKSAPI / KoikatuAPI (required, no minimum version is declared)
- ModBoneImplantor (required, no minimum version is declared)
- KKSABMX (optional, recommended for editable and saved bone offsets)
- Coordinate Load Option (optional)

## Installation

Place `KKS_AccessoryBoneBinder.dll` in:

```text
BepInEx/plugins/
```

Install KKSAPI and ModBoneImplantor as well. ABMX and Coordinate Load Option are soft dependencies: the basic bone attachment still works when they are absent.
...
4. Rename the placeholder to exactly match the target KKS body bone, including letter case.
...
In Studio, KKSPE does not read this plugin's data directly. ABMX first applies the saved modifier to the bound Transform; KKSPE then observes the resulting bone transform normally.
...
- The bone attaches but saved offsets do not apply: verify KKSABMX is loaded and edit the `ABB_Sxx_...` entry.
...
```powershell
dotnet build KKS_AccessoryBoneBinder.csproj -c Release
```
